...
- [injector.md](injector.md) — 规则引擎：Rule 字段、(op,path,off,length) 匹配、HealOnWrite 坏扇区
  按块治愈、spare 备用预算、Refresh 重置、在线管理 API（Add/Delete/Clear/List/Refresh/SetSpare/
  SetCapacity/SetProfile/SetSpeed）。
- [latency.md](latency.md) — 设备性能模型：LatencyProfile 字段、预设档（none/memory/ssd/hdd）、
  手动旋钮（rand=叠加增量/seq=速度上限）、speed 倍速、顺序/随机判定、per-byte 带宽、backing(tmpfs) 校准与钳制。
- [capacity.md](capacity.md) — 模拟容量（capacity）：挂载校验、运行时 write ENOSPC、statfs 反映，
  以及 backing 应为 tmpfs 的建议与不采用的后果。
