...
`Op` 取值：`OpOpen / OpOpendir / OpCreate / OpRead / OpReaddir / OpWrite / OpLookup / OpMkdir /
OpRmdir / OpUnlink / OpRename / OpGetattr / OpStatfs / OpSetattr / OpGetxattr / OpSetxattr /
OpRemovexattr / OpListxattr / OpFsync / OpFlush`；留空表示任意 op。
...
### 目录读取故障（`OpOpendir` / `OpReaddir`）

目录读取有两个注入点：`OpOpendir` 命中则 opendir 直接失败（不打开 backing 目录）；`OpReaddir`
让 opendir 成功、但读取条目（getdents）失败——更细粒度，模拟"打开正常、读到一半失败"。

```go
inj.Add(faultfs.Rule{Op: faultfs.OpOpendir, Path: "hash", Errno: syscall.EIO}) // ls mp/hash → EIO
inj.Add(faultfs.Rule{Op: faultfs.OpReaddir, Path: "hash", Errno: syscall.EIO}) // opendir ok、getdents EIO
```

> `OpReaddir` 推荐永久规则（`N:0`，默认）：go-fuse bridge 仅在每轮 READDIR 的首个 getdents
> 返 errno 时透传给内核，轮次中途的 errno 会被吞掉。`OpOpendir` 是单次 opendir、每次新调，
> 无此限制——这也正是把它作为目录注入首选的原因。
