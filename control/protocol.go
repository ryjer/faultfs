...
type Req struct {
	...
	SpareBlockSize int64   `json:"spare_block_size,omitempty"` // set-spare: 每块字节数（0→1）；>1 时治愈按 ceil(坏区/块) 整块消耗
	SkipLatency    bool    `json:"skip_latency,omitempty"`     // refresh: 跳过 profile/speed 复位（--keep-latency）
}
...
// What ∈ "rule"（含 ID）/ "spare" / "latency"；Before/After 为人类可读状态。
